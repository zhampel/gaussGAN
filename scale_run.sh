...
for ((idx=0; idx<15; idx=$((idx+3)))); do
    ...
    for dscale in ${dscale_list[@]}; do
        ...
        P1=$!
        python train.py -f /home/zhampel/gaussGAN/datasets/data_gauss_dim${dim}.h5 -n ${n_epochs} -c ${dscale} -d ${ldim_list[idx]} -s ${sig} -g 2 -w &
        P2=$!
        python train.py -f /home/zhampel/gaussGAN/datasets/data_gauss_dim${dim}.h5 -n ${n_epochs} -c ${dscale} -d ${ldim_list[idx]} -s ${sig} -g 3 -w &
        P3=$!
        wait $P1 $P2 $P3
    done
done
